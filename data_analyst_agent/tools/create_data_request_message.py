...
def create_data_request_message(
    ...
    Args:
        current_analysis_target: Analysis target identifier (cost center code, metric name, etc.)
        date_ranges: Dict with date range keys
        request_analysis: Dict with gl_accounts and analysis type
        target_label: Dataset-specific label for the analysis target (from contract.target_label)
    ...
    return f"""
[DATA REQUEST FOR {target_label.upper()}: {current_analysis_target}]

Please retrieve the following data:

1. FINANCIAL DATA (Account Research DS):
   - {target_label}: {current_analysis_target}
   - Date Range: {date_ranges['primary_query_start_date']} to {date_ranges['primary_query_end_date']}
   - GL Accounts: {gl_accounts_str}
   - Format: CSV export with columns: period, gl_account, amount, analysis_target
   - Tool: Use export_bulk_data_tool for efficiency

2. OPERATIONAL METRICS (Ops Metrics DS):
   - {target_label}: {current_analysis_target}
   - Date Range: {date_ranges['supplementary_query_start_date']} to {date_ranges['supplementary_query_end_date']}
...
3. ORDER DETAILS (conditional - only if needs_order_detail = true):
   - {target_label}: {current_analysis_target}
   - Date Range: {date_ranges['detail_query_start_date']} to {date_ranges['detail_query_end_date']}
