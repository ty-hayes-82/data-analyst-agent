...
def _materiality_weight(item_total: float, grand_total: float) -> float:
    ...
    if grand_total == 0 or item_total == 0:
        return 0.1  # floor so tiny items still appear if statistically extreme
    share = abs(item_total) / abs(grand_total)
    return min(0.1 + share * 3.0, 1.0)  # 33%+ share = max weight


def _composite_score(stat: float, mag: float, mat: float) -> float:
    """Combine the three dimensions into a single 0..1 score."""
    # Statistical significance is the primary driver; materiality scales it
...
def _compute_grand_total(stats: dict) -> float:
    ...
    if isinstance(ss, dict) and ss.get("grand_total"):
        try:
            ...
            pass
    totals = stats.get("monthly_totals", {})
    if totals:
        acc = 0.0
    ...
    if drivers:
        return sum(abs(d.get("avg", 0.0)) * d.get("count", 1)
                   for d in drivers if isinstance(d, dict))
    return 0.0


def _item_total_from_drivers(item_name: str, stats: dict) -> float:
    """Get the average or total for a specific item from top_drivers."""
    for d in stats.get("top_drivers", []):
        if d.get("item_name", d.get("item")) == item_name:
            return abs(d.get("avg", 0.0)) * d.get("count", 1)
    return 0.0
