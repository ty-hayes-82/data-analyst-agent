...
async def process_shippers_and_get_rates(
    ...
    Args:
        ...
        cost_center: Cost center code (e.g., "497", "067")
    ...
    Example:
        # Process shippers data from tableau agent
        process_shippers_and_get_rates(tableau_response, "497", 5.0)
    """
    try:
        project_root = Path(__file__).parent.parent.parent
        
        # Parse the shippers data from tableau agent
        print(f"[INFO] Parsing top shippers data for CC {cost_center}...")
        ...
        # Case 2: Object with 'top_shippers' key (from get_top_shippers_by_miles tool)
        ...
        output_file = output_dir / f"major_shippers_with_rates_{cost_center}.json"
...
async def get_major_shippers_with_contract_rates(
    ...
    Example:
        # Get major shippers and their rates for June 2025 at CC 497
        get_major_shippers_with_contract_rates("2025-06-01", "2025-06-30", "497", 5.0)
    """
    try:
        project_root = Path(__file__).parent.parent.parent
        
        # Step 1: Get top shippers by miles
        print(f"[INFO] Step 1: Getting top shippers for CC {cost_center}...")
        shippers_result = await get_top_shippers_by_miles(
        ...
        output_dir = project_root / "outputs"
        output_dir.mkdir(exist_ok=True)
        output_file = output_dir / f"major_shippers_with_rates_{cost_center}_{start_date}_{end_date}.json"
