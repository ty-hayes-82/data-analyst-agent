...
def _check_business_context_suppression(alert: dict, suppression_rules: dict, cost_center: str = None, period: int = None) -> tuple:
    """
    ...
    for rule_id, rule in suppression_rules.items():
        ...
        affected_cost_centers = rule.get("cost_centers", ["*"])
        if cost_center:
            cc_match = any(_matches_pattern(cost_center, cc_pattern) for cc_pattern in affected_cost_centers)
            if not cc_match:
                continue
...
async def apply_suppression(data: str) -> str:
    ...
    try:
        ...
        cost_center = input_data.get("cost_center")
        ...
        for alert in alerts:
            ...
            if business_context_rules:
                should_suppress, reason, rule_id = _check_business_context_suppression(
                    alert, business_context_rules, cost_center, period
                )
                if should_suppress:
